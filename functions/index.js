...
// Removed test function - keeping only production-ready code
...
async function getMarketIntelligenceDataTest(sport, team1, team2) {
  try {
    ...
    const marketData = {
      ...
      oddsTable: formatOddsTableTest(bookmakers),
...
function calculateBestLinesTest(bookmakers, event) {
  ...
  // FORMULA 3: Find best spread lines (prioritize point first, then price)
...
function formatOddsTableTest(bookmakers) {
  return bookmakers.slice(0, 8).map(bookmaker => ({
    bookmaker: bookmaker.title,
    bookmakerKey: bookmaker.key,
...
function calculateEVOpportunities(bookmakers, event) {
  ...
  // Calculate true fair value (vig removed)
  const fairValue = {
  ...
  const vigAnalysis = {
    moneyline: {
      sharp: fairValue.moneyline.home && fairValue.moneyline.away ?
        calculateMarketVig(fairValue.moneyline.home, fairValue.moneyline.away) : null,
    ...
    spread: {
      sharp: fairValue.spread.home && fairValue.spread.away ?
        calculateMarketVig(fairValue.spread.home, fairValue.spread.away) : null,
      market: calculateMarketVig(
    ...
    total: {
      sharp: fairValue.total.over && fairValue.total.under ?
        calculateMarketVig(fairValue.total.over, fairValue.total.under) : null,
      market: calculateMarketVig(
  ...
  // Check moneyline +EV using TRUE FAIR VALUE (vig removed)
  if (fairValue.moneyline.fair1 && fairValue.moneyline.fair2) {
    moneylineOdds.home.filter(o => !o.isSharp).forEach(book => {
    ...
    moneylineOdds.away.filter(o => !o.isSharp).forEach(book => {
  ...
  // 5. Arbitrage Detection with Bet Sizing
  const bestHomeOdds = Math.max(...moneylineOdds.home.map(o => o.odds));
  const bestAwayOdds = Math.max(...moneylineOdds.away.map(o => o.odds));
  const bestHomeBook = moneylineOdds.home.find(o => o.odds === bestHomeOdds);
  const bestAwayBook = moneylineOdds.away.find(o => o.odds === bestAwayOdds);

  const arbCheck = (1 / bestHomeOdds) + (1 / bestAwayOdds);
  const hasArbitrage = arbCheck < 1.0;
